...
def _check_nan_excessive(dense: np.ndarray, max_nan_ratio: float = 0.3) -> bool:
    """Check if overall NaN ratio exceeds threshold.
    
    Returns True if the sample should be dropped.
    Per 方法.docx: missing values → delete the sample.
    
    Note: Checks overall (layer-pooled) NaN ratio, not per-layer.
    Sparse NEXRAD radar has naturally empty upper layers regardless of storm.
    """
    nan_ratio = np.isnan(dense).mean()
...
class DataPreprocessor:
    ...
    def __init__(self, config: dict):
        ...
        self._nc_lock = threading.Lock()          # HDF5/netCDF4 非线程安全，NC 文件操作串行化
    ...
    def process_events(self, nexrad_dir: str, tracks_dir: str,
        ...
        # Extract available dates from NC filenames (e.g. nexrad_..._20230102T143000Z.nc)
        nc_dates = {f.stem.split('_')[-1][:8] for f in nc_files.values()
                     if len(f.stem.split('_')[-1]) >= 8}

        # Pre-load coordinates from first file (shared read-only across threads)
        self._load_coordinates(list(nc_files.values())[0])

        # 2. Parse CSV files ONLY for dates with NC data
        all_storms = self._parse_all_csvs(tracks_dir, nc_dates)
    ...
    def _parse_all_csvs(self, tracks_dir: str,
                        nc_dates: set[str] | None = None
                        ) -> dict[str, list[dict]]:
        """Parse CSV files, group by Storm Number + date.

        If nc_dates is provided, only CSV files matching those dates are processed.
        ...
        # Filter CSV files to dates with NC data
        for csv_file in sorted(csv_dir.glob("*.csv")):
            csv_date = csv_file.stem  # yyyymmdd
            if nc_dates is not None and csv_date not in nc_dates:
                continue  # skip dates without radar data
            try:
    ...
    def _extract_radar_crop(self, nc_file: Path, center_lon: float,
        ...
        # HDF5/netCDF4 C lib is NOT thread-safe; serialize NC file access
        with self._nc_lock:
